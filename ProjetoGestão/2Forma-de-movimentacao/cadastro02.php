...
<?php
      ...
      $codigo= $_POST ["codigo02_cad"];//atribuição do campo "nome" vindo do formulário para variavel
      $codigoMovi=$_POST["codigomovi_cad"]
      $movimento= $_POST ["descricao_cad"];//atribuição do campo "email" vindo do formulário para variavel 
      //$ddata=date("Y-m-d H:i:s");

         
      $query = "INSERT INTO `formamovi` (`codigo`,`Codigomovimento`,`descricao` ) "
            . "VALUES ('$codigo','$codigoMovi','$movimento' )";

      $formamovi = $conn -> prepare ($query);
      
      if($formamovi->execute()){
